  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {Object.entries(grouped).map(([categoryName, items]) => {
        ...
        const totalTransactions = items.reduce((sum, d) => sum + d.total_transactions, 0);
          ...
          <Card key={categoryName} className="flex flex-col">
            ...
            {items.length === 1 && (
              <CardContent className="pt-0 pb-4">
                <Badge variant={statusConfig[items[0].status_cashflow]?.variant ?? "outline"} className="text-xs">
                  {statusConfig[items[0].status_cashflow]?.label ?? items[0].status_cashflow}
                </Badge>
              </CardContent>
            )}
          </Card>
